...
- **Limitation discovered during review:** the GitHub Connector used for review submission returned `403 Resource not accessible by integration` / fell back to `COMMENT` when attempting a native `APPROVE`/`REQUEST_CHANGES` review state on a PR the same account owns. See `docs/KNOWN_LIMITATIONS.md` for the workaround this log's own review cycle used (a `COMMENT` review whose body states "Substantive decision: APPROVE").
