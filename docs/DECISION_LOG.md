...
- **Limitation discovered during review:** the GitHub Connector used for review submission returned `403 Resource not accessible by integration` when attempting to submit a formal Pull Request Review. See `docs/KNOWN_LIMITATIONS.md` for the two-tier fallback policy this discovery led to (formal browser-submitted `COMMENTED` review, then last-resort PR Conversation comment). This review cycle's own workaround at the time was a top-level PR Conversation comment whose body stated "Substantive decision: APPROVE" — under the now-corrected policy this was the last-resort tier, not a substitute for a formal review; it is retained here as an accurate record of what happened, not as an example to repeat.
...
- **Status:** Draft, pending review, including one review-fix round before merge:
  ...
  - `docs/KNOWN_LIMITATIONS.md` used unsupported "silently downgrade" wording for the GitHub connector's review-submission behavior. Fixed to describe the actual observed `403` failure, and that reading PRs/diffs/reviews/CI is unaffected.
