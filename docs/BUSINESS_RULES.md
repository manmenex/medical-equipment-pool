...
## Roles and the confirmed 3-role permission matrix (Roadmap PR10, in progress)

**Not yet merged as of the current baseline above** — recorded here because the confirmed matrix is now design-approved and implemented on a Draft PR; this section documents the target state, not the state at the baseline commit. The legacy 5-role model (`admin`, `biomedical_engineer`, `ward_nurse`, `transport_staff`, `viewer`) is replaced everywhere a role is persisted or checked by exactly three confirmed roles:
