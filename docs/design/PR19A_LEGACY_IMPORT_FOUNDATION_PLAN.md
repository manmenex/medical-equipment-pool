...
| `status` | VARCHAR(20) | NOT NULL | `'registered'` | `CHECK status IN ('registered', 'frozen')` — the source lifecycle, §6. This column alone is the CAS predicate correction (§15.2) and freeze (§6) both contend on; no separate `version` counter exists on this table (unlike `import_sessions.version`, §7) — `status` is a fully sufficient guard here, since there are only two states and the `'registered' → 'frozen'` transition is one-directional and irreversible |
...
- **Changes source to `SOURCE_FROZEN`:** `status = 'frozen'`, `frozen_at = now()` — the single `WHERE status = 'registered'` predicate is the entire CAS guard; `import_sources` carries no separate `version` counter (§4.2), since `status` alone is sufficient for a two-state, one-directional, irreversible transition.
...
6. **"Best-effort" means exactly this:** step 3's publication is not guaranteed to succeed, and its failure to do so — whether from another fencing loss (step 5) or from `TX2` itself hitting a further infrastructure error (step 7 below) — is not escalated, retried inline, or resolved with an unconditional/unfenced write. A worker that cannot cleanly publish its own failure simply gives up; the job is left `'running'` (if `TX2` never even attempted) or correctly fenced-out (if it did), and ordinary lease-expiry recovery (§9.3) closes it out exactly as it would for any other unresponsive worker (§9.5).
7. **`TX2` infrastructure failure is a distinct outcome from fence loss (step 5), with its own authoritative contract.** Step 5 covers a *clean* fencing result — `TX2`'s `UPDATE`s ran and returned zero rows because another owner already holds the job. This step covers the opposite case: `TX2` cannot be opened at all, or raises an infrastructure-level exception (database unavailable, connection loss, the process terminating) before either `UPDATE` commits — no clean fencing result was ever obtained, because `TX2` never got the chance to run to completion. The two must never be conflated:
   - **The response is the repository-standard centralized `500 INTERNAL_ERROR` envelope** (`backend/app/main.py`'s generic unhandled-exception handler; `docs/api/ERROR_CODES.md`'s response envelope) — **never `409 IMPORT_RECOVERY_REQUIRED`.** Infrastructure failure is not a recovery, fencing, or ownership conflict; nothing was actually lost to a competing owner — `TX2` simply never ran to a result.
   - **No unfenced failure state is published.** There is no fallback, unconditional `UPDATE` that forces the job to `'failed'` outside the fencing predicate to compensate for `TX2`'s own failure — that is exactly the unsafe pattern §9.4.1 step 6 already rules out for the success path, and it is ruled out here identically.
   - **The job and session are left exactly as they were:** `'running'` / `*_RUNNING`. From the database's perspective this is indistinguishable from a worker that never attempted `TX2` at all — the same state §9.5's "process does not survive" rows already describe.
   - **The lease is retained, not explicitly released,** and expires naturally at `lease_expires_at` per §9.1/§9.2 — this failure changes nothing about lease timing.
   - **Recovery (§9.3) acquires ownership of the job only after that lease actually expires** — never earlier, and never as a side effect of the `500` response itself, which performs no database mutation of its own.
   - **Recovery then performs the next fenced publication for this job** — the identical claim-and-transition contract §9.3 already defines for any other lease-expiry recovery. No new recovery mechanism exists for this case; it reaches the existing §9.3 contract by a distinct path, not a different one.
8. **The response the caller receives depends on which of steps 4/5/7 actually happened — never a blanket code:**
   - **`TX2` commits (step 4, the common case):** this attempt's own failure was recorded cleanly; no fencing conflict occurred at all. The response is the phase-appropriate outcome, identical to any other completed attempt: `validate` → `200`, session `status = VALIDATION_FAILED`; `dry-run` → `200`, session `status = DRY_RUN_FAILED` (§16); `execute` → `500 IMPORT_EXECUTION_FAILED` (§23) — execute's own runtime failure is the one phase treated as a genuine server error rather than a normal completed-but-failed outcome, since a partially-attempted execution has no equivalent of "just re-validate and try again" the way validate/dry-run do.
   - **`TX2` itself is fenced out (step 5):** only this outcome — a genuine stale-lease, fence-loss, or recovery-ownership conflict — returns `409 IMPORT_RECOVERY_REQUIRED`, for any phase. Never for a cleanly-recorded failure (the branch above), and never for infrastructure failure (the branch below).
   - **`TX2` itself fails on infrastructure grounds (step 7):** returns `500 INTERNAL_ERROR` (the repository-standard envelope) — never `409 IMPORT_RECOVERY_REQUIRED`, since nothing was fenced out; `TX2` never produced a fencing result to lose.
   - In no outcome is a success response returned for domain work that was rolled back, and in no outcome is a fencing loss or an infrastructure failure met with silence.
...
**This table is §9.4.2's failure contract applied to each concrete failure mode — it introduces no new behavior of its own.** The controlling distinction is whether the worker *process* survives the failure:

- **Process survives** (a PostgreSQL-level abort, a serialization failure, a deadlock, a statement timeout, or a caught Python/domain exception): the worker immediately runs §9.4.2's own path itself — `TX1` rolled back, session disposed, `TX2` attempts fenced failure publication. This is the common, fast path and normally resolves the attempt **without any operator action**; `/recover` is not required.
- **Process does not survive** (worker crash, application restart, or a request/connection torn down before `TX2` can run): no code the worker owns ever executes `TX2` — there is no process left to run it. The job simply stays `'running'` until `/recover` (§9.3) claims it. This is the *only* class of failure where recovery is the actual resolution path, not a fallback for something the worker could otherwise have done itself.

| Failure mode | Heartbeat lifetime | Lease release | CAS/version handling | Session state after | Audit event | Retry eligibility |
|---|---|---|---|---|---|---|
| PostgreSQL transaction abort (mid-work, **process survives**) | Renewal loop keeps renewing independently on its own session/transaction, unaffected by `TX1`'s own abort | Not explicitly released by the abort itself; `TX2`'s fenced failure `UPDATE` (§9.4.2 steps 3–4) sets job `status='failed'` immediately in the common case, ending the running lease without ever waiting for `lease_expires_at` | `TX1` rolled back (domain writes discarded, §9.4.2 step 1); `TX2` attempts the same fencing tokens and normally succeeds (§9.4.2 step 4) | `*_FAILED` immediately via `TX2` in the common case; stays `*_RUNNING` awaiting `/recover` only if `TX2` itself cannot complete safely (the worker also dies before `TX2` commits, or `TX2` is fenced out, §9.4.2 step 5) | None in the common case (a cleanly-recorded failure is not an audited event, §19); `AUDIT_ACTION_IMPORT_FENCE_LOST` only if `TX2` is fenced out; `AUDIT_ACTION_IMPORT_RECOVERY` only if the worker never reaches `TX2` at all and an operator later recovers it | New attempt immediately once `TX2` commits — no recovery required in the common case (§5) |
| Serialization failure (**process survives**) | Same as PostgreSQL transaction abort — the losing transaction is aborted exactly like any other database-level abort; the process survives and follows the identical `TX1`-rollback/`TX2` path | Same | Same | Same | Same | Same |
| Deadlock (**process survives**) | Same as PostgreSQL transaction abort — PostgreSQL aborts one of the two deadlocked transactions; the aborted one is handled exactly like any other database-level abort above | Same | Same | Same | Same | Same |
| Timeout — statement-level (**process survives**) | Same as PostgreSQL transaction abort — the statement is aborted, but the process keeps running and follows the identical `TX1`-rollback/`TX2` path | Same | Same | Same | Same | Same |
| Timeout — request/connection-level (**process does not survive** to attempt `TX2`) | Renewal loop dies with the torn-down request — no further renewals | Not explicit — expires naturally at `lease_expires_at`, since neither `TX1`'s rollback nor any `TX2` attempt can complete | Completion never attempted — no process remains to run `TX1`'s rollback or `TX2` | Stays `*_RUNNING` until `/recover` | `AUDIT_ACTION_IMPORT_RECOVERY` once recovered | New attempt after recovery, per §5 |
| Worker crash (process death — **process does not survive**) | Renewal loop dies with the process — no further renewals | Not explicit — expires naturally | Completion never attempted — no process remains to run `TX1`'s rollback or `TX2` | Stays `*_RUNNING` until `/recover` | `AUDIT_ACTION_IMPORT_RECOVERY` once recovered | New attempt after recovery, per §5 |
| Heartbeat loss (renewal `UPDATE` raises, worker process still alive) | Bounded retry — up to 3 consecutive transient failures (§9.2) before giving up | Not explicit unless the underlying cause is an actual lease reassignment, detected via a clean 0-row response | The worker's own eventual `TX1`/`TX2` attempt (§9.4.1 or §9.4.2) proceeds normally if it reaches completion before recovery fires; fenced out (§9.4.2 step 5) only if recovery wins the race first | Depends on whether recovery fires before the worker's own attempt completes | `AUDIT_ACTION_IMPORT_RECOVERY` if recovered first; `AUDIT_ACTION_IMPORT_FENCE_LOST` if the worker's own attempt is fenced out instead | Same |
| Lease expiration (worker unaware, or aware but unable to renew, and no `TX1`/`TX2` attempt reaches completion before the deadline) | N/A | Not actively released — recovery's claim (§9.3) treats an expired, still-`'running'` lease as claimable | Recovery's own CAS (§9.3) | Transitions via `/recover` | `AUDIT_ACTION_IMPORT_RECOVERY` | New attempt after recovery |
| Application restart (deployment restart, rolling update — **process does not survive**) | All in-flight renewal loops die with the process — identical to worker crash | Same as worker crash | Same as worker crash — no process remains to run `TX1`'s rollback or `TX2` | Same as worker crash | Same as worker crash | Same as worker crash |
...
**This guarantees exactly one terminal execution outcome per job — not exactly one audit event.** The two are different invariants and must not be conflated. For any single job, at most one of {the worker's own completion write (§9.4.1/§9.4.2), a recovery claim (§9.3)} can ever **write the job's terminal status** (`succeeded`/`failed`/`abandoned`), because both are gated on the identical predicate `status = 'running'` on the same row, and PostgreSQL serializes concurrent `UPDATE`s to one row. Whichever commits first flips `status` away from `'running'`, which structurally excludes the other from ever matching its own `WHERE` clause afterward — this is the one-terminal-outcome guarantee, and it holds without exception.

**It does not follow that only one audit event is ever written for that job.** If recovery wins first, it writes `AUDIT_ACTION_IMPORT_RECOVERY`; the worker's own later attempt, now fenced out, independently writes `AUDIT_ACTION_IMPORT_FENCE_LOST` when it discovers this — two distinct, legitimate audit entries describing two distinct facts about the same job's lifecycle (the job was recovered; a later attempt was correctly rejected), neither a duplicate of the other. §9.6 Diagram 7 illustrates exactly this sequence: one `AUDIT_ACTION_IMPORT_RECOVERY` entry, followed by one `AUDIT_ACTION_IMPORT_FENCE_LOST` entry, for the same job. What the structural guarantee above actually rules out is narrower and more specific: two audit events both purporting to record the *same* fact — e.g., two `AUDIT_ACTION_IMPORT_RECOVERY` entries for one recovery claim, or a worker's completion write being recorded as both a success and a fenced-out failure. Recovery audit and fence-lost audit may coexist for one job without violating the one-terminal-execution-outcome invariant, because they describe two different, sequential events, not two competing claims to have produced the job's terminal state.
...
8. **`POST /import-sessions/{id}/validate`** — No request body (no parser exists in this foundation). Response: `ImportSessionOut`. Codes: `200` (both a fresh `VALIDATED` result and a cleanly-recorded `VALIDATION_FAILED` outcome, §9.4.2 step 8 — a validation attempt that runs to completion, whether it finds blocking errors or crashes and is cleanly recorded via `TX2`, is not itself an HTTP error), `404`, `409 IMPORT_SOURCE_NOT_REGISTERED` (§6), `409 IMPORT_SESSION_INVALID_STATE`, `409 IMPORT_ATTEMPT_IN_PROGRESS`, `409 IMPORT_RECOVERY_REQUIRED` (pre-check *or* post-completion fencing loss, §9.4.2 step 5 — never for a cleanly-recorded failure), `422 IMPORT_ADAPTER_NOT_REGISTERED`.
...
10. **`POST /import-sessions/{id}/dry-run`** — Response: `ImportSessionOut`. Codes: `200` (both a fresh `DRY_RUN_COMPLETED` result and a cleanly-recorded `DRY_RUN_FAILED` outcome, §16, §9.4.2 step 8), `404`, `409 IMPORT_SESSION_INVALID_STATE`, `409 IMPORT_ATTEMPT_IN_PROGRESS`, `409 IMPORT_RECOVERY_REQUIRED` (pre-check *or* post-completion fencing loss, §9.4.2 step 5 — never for a cleanly-recorded failure), `501 IMPORT_ADAPTER_NOT_IMPLEMENTED`.
11. **`POST /import-sessions/{id}/execute`** — Response: `ImportSessionOut`. Codes: `200` (fresh success or idempotent replay), `404`, `409 IMPORT_SESSION_INVALID_STATE`, `409 IMPORT_ATTEMPT_IN_PROGRESS`, `409 IMPORT_RECOVERY_REQUIRED` (pre-check *or* post-completion fencing loss, §9.4.2 step 5 — never for a cleanly-recorded failure), `501 IMPORT_ADAPTER_NOT_IMPLEMENTED`, `500 IMPORT_EXECUTION_FAILED` (a cleanly-recorded runtime failure via `TX2`, §9.4.2 step 8 — the one phase where a completed-but-failed attempt is itself the HTTP error, unlike validate/dry-run). Audit: one `AUDIT_ACTION_IMPORT` entry on fresh success only.
...
- How a `TX2` infrastructure failure differs from a fence loss → §9.4.2 step 7 (`500 INTERNAL_ERROR`, session left `*_RUNNING`, lease expires naturally, recovery owns the next fenced publication — never `409 IMPORT_RECOVERY_REQUIRED`)
