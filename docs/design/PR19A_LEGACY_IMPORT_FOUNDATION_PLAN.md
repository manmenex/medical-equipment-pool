...
- **Increments `import_sources.version`:** yes, `version = version + 1`, in the same `UPDATE` that sets `status = 'frozen'` — this is the same column (not to be confused with the unrelated, caller-supplied `source_version` field), and the same CAS discipline, that source correction (§15.2) also writes through, so freeze and correction structurally contend on one predicate.
...
7. **The response the caller receives depends on which of steps 4/5 actually happened — never a blanket code:**
   ...
   - **`TX2` itself is fenced out (step 5):** only this outcome — a genuine stale-lease, fence-loss, or recovery-ownership conflict — returns `409 IMPORT_RECOVERY_REQUIRED`, for any phase. Never for a cleanly-recorded failure (the branch above).
   - In no outcome is a success response returned for domain work that was rolled back, and in no outcome is a fencing loss met with silence.
