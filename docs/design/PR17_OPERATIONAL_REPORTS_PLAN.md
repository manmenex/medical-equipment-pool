...
*Trade-off:* the response shape carries every `TransactionOut` field (some irrelevant per report, e.g. `dispatch_type` on a Receive Report row), not a report-trimmed subset. Accepted: introducing report-specific slim schemas now would be premature schema proliferation for a shape the frontend can trivially select fields from; can be revisited if PR18's export format needs a narrower shape. **Revised per review finding PR17-M1R:** `TransactionOut` itself gains two new, additive, read-only fields — `dispatch_operator_display_name`/`receipt_operator_display_name` (§10.1, §11) — so report rows can actually show who performed the event, not only filter by them; this is the answer to "whether report rows need an event-specific operator summary schema," resolved as *extend the existing shared schema*, mirroring exactly how PR16 itself incrementally added `dispatch_business_date`/`receipt_business_date` to the same schema, rather than inventing a parallel report-only response type.
...
| **Response schema** | `Page[TransactionOut]` — reused, plus two new read-only fields on `TransactionOut` itself (§8, §11, corrected per review finding PR17-M1R): `dispatch_operator_display_name: str \| None` and `receipt_operator_display_name: str \| None`, resolved from the same bounded operator identity source as §10.4. The Receive Report's rows always have `receipt_operator_display_name` populated (every row it returns is `CLOSED`, §7.1) and `dispatch_operator_display_name` populated whenever `borrower_user_id` was recorded. |
...
- **Visibility (corrected per review finding PR17-M1R — the prior claim of "no new information-exposure risk" was inaccurate and is withdrawn):** `TransactionOut` gains two genuinely new fields (`dispatch_operator_display_name`/`receipt_operator_display_name`, §8/§10.1/§11) and `GET /report-options/operators` (§10.4) is a genuinely new lookup surface — today, no endpoint exposes any operator's identity for a dispatch or receipt event at all. This is a deliberate, bounded expansion, not an oversight: see §10.4's "Privacy / authorization rationale" row for the full justification (bounded to real recorded operators only, name/id/active-flag only, serves the cited §6.1/§6.2 operational need, consistent with `docs/GLOSSARY.md`'s existing "Operator" concept). The existing equipment response shape (Verify Checklist, interpretation A) still introduces no new field, unchanged from the prior revision.
...
**Two fix rounds — review `4804227912` (PR17-H1/H2/H3, PR17-M1) and incremental review `4804643991` (PR17-M1R, PR17-H4, and a §7.1 wording contradiction) — have now been resolved. Owner Decision #1 below is explicitly preserved, unresolved, and unaltered across both rounds — not addressed, guessed, or narrowed, per this task's explicit instruction in each round.**
...
- Report rows (`TransactionOut`, §8/§10.1/§11) correctly show `dispatch_operator_display_name`/`receipt_operator_display_name` for every row where the corresponding user FK is set, and `None` where it is not.
...
Verified before this second revision was finalized (incremental review `4804643991`, findings PR17-M1R/PR17-H4, plus the §7.1 minor wording contradiction):
...
- [x] No API implementation changed — every endpoint in `backend/app/api/v1/transactions.py`, `backend/app/api/v1/reports.py`, `backend/app/api/v1/users.py`, and every other existing router is untouched; §10's endpoints (including the revised §10.4) are proposals only.
...
- [x] Only reportable operators are exposed — §10.4's Source / population rule bounds the response to `User` rows actually referenced by `borrower_user_id`/`received_by_user_id`, never every `User` row; verified against every table row (Source, Response fields, Active/inactive, Search, Pagination) and §9's filter description.
- [x] The operator lookup is no longer a user directory — §10.4's own opening paragraph, §21's new Out of Scope bullet, and §14's corrected Visibility bullet all state this explicitly; `UserOut`'s full field set (`employee_code`/`email`/`phone`/`role`) remains excluded, unchanged from the prior round.
- [x] Governance completion requires the entire PR17 scope — §17's Final Slice now states its two explicit preconditions (Owner Decision #1 = A and Slice 4 merged, or Owner Decision #1 = B/C and a separate approved scope amendment); §20's new "Governance completion" acceptance criteria and §21's new Out of Scope bullet restate the same rule; §22 cross-references it.
- [x] §7.1 matches every other Receive Report definition — the "not a special-case exclusion... same NULL-propagation behavior already shipped" sentence (which contradicted the unconditional `require_receipt` predicate introduced in the prior round) is replaced with wording that correctly distinguishes PR16's conditional NULL-propagation from PR17's new, always-on invariant; no other sentence in §5, §7.1, §8, §10.1, or §11 makes the old, now-contradicted claim (verified by re-reading all four in full).
- [x] Owner Decision #1 (§18) remains open — unaltered content; the new note above Owner Decision #1 from the prior round is itself also unaltered.
- [x] Printing remains deferred to PR18 — unaffected by this round's changes (§13, §17, §21).
- [x] `git diff --check` passes — no whitespace errors.
- [x] Design documents internally consistent — every new/revised claim in this round (§7.1, §8, §9, §10.1, §10.4, §11, §14, §17, §18 Owner Decision #2, §19, §20, §21, §22, §24) cross-references the same corrected source of truth (§10.4) rather than restating it independently.
- [x] All prior-round blockers (PR17-H1/H2/H3/M1) remain resolved and unregressed by this round's edits.

---

## 24. Deliverables

1. **Files changed:** `docs/design/PR17_OPERATIONAL_REPORTS_PLAN.md` (this document) — the only file modified by this second incremental fix.
2. **Findings resolved, section by section:** PR17-M1R -> §9, §10.4, §11, §14, §18 Owner Decision #2, §19, §20, §21, §23; PR17-H4 -> §17 Final Slice, §20, §21, §22, §23; §7.1 minor wording contradiction -> §7.1.
3. **Updated operator lookup semantics (§10.4):** `GET /api/v1/report-options/operators` now returns only the distinct `User` rows actually referenced by `borrower_user_id`/`received_by_user_id` in transaction history (a bounded `SELECT DISTINCT` over the union of both columns) — never every `User` row. Genuinely paginated (`limit`/`cursor`, `full_name ASC, id ASC`), not unpaginated. Same minimal `id`/`display_name`/`is_active` fields as before. The endpoint's "Privacy / authorization rationale" row now documents the genuine new PII exposure and its justification explicitly, rather than claiming no new risk exists. Report rows (`TransactionOut`) also gain `dispatch_operator_display_name`/`receipt_operator_display_name` (§8, §10.1, §11) so reports can display, not only filter by, the operator.
4. **Updated governance completion rule (§17, Final Slice):** the Final Slice may only run once every one of Receive, Issue, and Equipment Verify Checklist is resolved — either Owner Decision #1 = A and Slice 4 merged, or Owner Decision #1 = B/C and a separately approved governance decision formally amends PR17's scope. Slices 1-3 remain independently mergeable on their own merits, but merging them alone never triggers a "Roadmap PR17 complete" governance sync or advances the next-planned-item to PR18.
5. **Corrected Receipt Report canonical wording (§7.1):** the sentence claiming OPEN-transaction exclusion was "not a special-case exclusion this design adds... the same NULL-propagation behavior already shipped" — which contradicted the unconditional `require_receipt` predicate introduced in the first fix round — is replaced with wording that correctly distinguishes PR16's conditional NULL-propagation (only active when a `business_date`/`shift` filter is present) from PR17's new, always-on invariant. No other section repeats the retracted claim.
6. **Confirmation no implementation changed:** confirmed — `git status --short` shows exactly one modified file; no source code, test code, migration, API implementation, or frontend implementation changed (§23).
7. **Validation results:** §23.
8. **New head SHA:** recorded in the PR's own commit/push metadata (outside this document's scope to self-report).

*No production code was written or modified to produce this revision. No migration was generated. No application file was modified. No reporting library was introduced. Every claim about existing code cites the specific file inspected (§2), and the additional citations from this and the prior round — `backend/app/api/v1/master_data.py::list_wards`, `backend/app/api/v1/users.py`, `backend/app/schemas/master_data.py::UserOut`, `backend/app/models/user.py` — not assumption.*
