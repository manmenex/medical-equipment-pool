...
*Trade-off:* the response shape carries every `TransactionOut` field (some irrelevant per report, e.g. `dispatch_type` on a Receive Report row), not a report-trimmed subset. Accepted: introducing report-specific slim schemas now would be premature schema proliferation for a shape the frontend can trivially select fields from; can be revisited if PR18's export format needs a narrower shape. **Corrected per review finding PR17-H5 (superseding the PR17-M1R revision, which incorrectly proposed extending `TransactionOut` itself — see §11's Architecture layering).** The prior revision of this document added `dispatch_operator_display_name`/`receipt_operator_display_name` directly to the shared `TransactionOut` schema — this was a genuine defect: `TransactionOut` also backs the existing, unmodified `GET /transactions` and `GET /transactions/{id}` (§2), so extending it would have silently changed those endpoints' contracts too, directly contradicting this same document's own "No existing endpoint's contract... changes in any respect" claim (§20). The answer to "whether report rows need an event-specific operator summary schema" is still **yes**, but the mechanism is corrected: a new, **report-only** schema, `ReportTransactionOut`, which extends `TransactionOut` (inherits every existing field unchanged, adds the two operator-display fields on top) — used exclusively by `GET /reports/receive`/`GET /reports/issue` (§10.1, §10.2). `TransactionOut` itself, and every field it already had, is untouched; `GET /transactions`/`GET /transactions/{id}` continue returning exactly what they return today.
