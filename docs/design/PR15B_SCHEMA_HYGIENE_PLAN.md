...
**Status:** Design only, committed and published for independent architecture review as Draft PR #52 (`docs/pr15b-schema-hygiene-design-review`, documentation-only — no production code, no migrations). **Architecture decisions approved to date:** FK policy = `RESTRICT` (§4); Timezone = Option A (§3.5); Migration split = three migrations numbered `0012`–`0014` (§8.2). **This round's revision resolves a primary blocker identified in review:** the prior migration strategy assumed every migration always transforms a pre-existing "before" schema, but `0001_initial.py` builds fresh installs from *current* ORM metadata, not a frozen snapshot — meaning a fresh install taken after this proposal's required ORM changes land already reaches the target schema before Migrations A/B/C run at all. §8.7/§8.7a now document a three-way verify-and-no-op policy (needs-transformation / already-at-target / neither) so every migration checks catalog state rather than assuming it. Implementation remains blocked pending full review sign-off — see the open items still tracked in §9.
...
This document proposes **no implementation**. Architecture review to date has approved: (1) `RESTRICT` as the default FK policy, (2) Option A for the timezone conversion (fix column type and Python callable only, no `server_default` convergence in this pass), and (3) the three-migration split (`0012` timezone / `0013` FK policy / `0014` index naming). Still open, and still gating implementation: the three named FK relationships requiring an explicit `RESTRICT`-vs-`SET NULL` decision (§9.1), confirmation that no further CHECK-constraint sweep or `PMSchedule`/`CalibrationSchedule` removal is in scope, and this round's revisions — the corrected historical-`due_at` treatment (§3.2/§3.4a), the expanded catalog-aware migration strategy (§8.7), and the new index-naming convergence section (§6.4).
...
If PR15B proceeds with converting the five columns identified above to `timestamptz` (`due_at` excluded — see §3.4a), the interpretation of **existing stored values** is stated explicitly: every naive value in every one of these five columns today was written by `datetime.utcnow()` (confirmed: no other write path exists for any of them — see §3.2's call-site citations), which — despite being naive — always represents a UTC instant; the bug is the missing timezone label, not incorrect wall-clock math. The correct, meaning-preserving conversion is therefore:
...
| Fresh database (`base → head`) | `ix_` from `0001` onward | `uq_` from their introducing migration onward | `idx_` created by `0001`/`0004`/`0007`, then renamed to `ix_` by `0014` — same transient-then-final path as §8.7's Path 1 | `_key` created implicitly by `0001`, then renamed to `uq_` by `0014` |

Both database paths converge on the same final names (the bottom two rows agree in every column) precisely because Migration C never assumes a name — it inspects `pg_indexes`/`pg_constraint` immediately before renaming (§6.3's fail-closed preflight) and acts on whatever it actually finds, whether that state was reached via the fresh path or the historical path.
...
- **FK `ondelete` changes** require no *data* preflight — no existing row can violate a delete-behavior change, since the change only affects what happens on a future `DELETE` that has never occurred (§4) — but do require the §8.7a catalog-*state* check (`pg_constraint.confdeltype`) to decide transform vs. no-op.
...
**Path 1 — fresh database (revised this round — the prior description was incomplete).** A brand-new PostgreSQL instance runs `alembic upgrade head`, which executes every migration from `0001` through `0014` (assuming Migrations A/B/C land as `0012`/`0013`/`0014`) in sequence, starting from nothing. Migration `0001_initial.py` bootstraps every table via `Base.metadata.create_all(bind=bind)` — meaning the *starting point* for a fresh install is whatever **the current ORM model declarations say at the moment the fresh install runs**, not a fixed historical snapshot frozen at `0001`'s authoring time.

This has a consequence the prior round of this document missed: **once the ORM model changes this proposal requires — the `DateTime(timezone=True)` type argument (§3.5), the `ForeignKey(ondelete=...)` parameters (§4), and the explicit `UniqueConstraint`/`naming_convention` names (§6.4) — actually land in `app/models/*.py`, `0001_initial.py`'s `create_all()` produces the *target* schema immediately, for every fresh install from that point forward.** A fresh database therefore does **not** pass through a naive/`NO ACTION`/auto-named "before" state at all after this proposal ships — it is born already in the state Migrations A/B/C exist to produce. The prior version of this section assumed every fresh install transiently exists in the pre-migration state before `0012`–`0014` run; that assumption only held before the accompanying ORM changes landed, and stops holding the moment they do — which is exactly when Migrations A/B/C first need to run against a fresh install. **This is why Migrations A/B/C cannot unconditionally assume "the before state exists and must be transformed" — see §8.7a's verify-and-no-op policy, which resolves this by checking rather than assuming.**

**Path 2 — historical upgrade.** An existing, already-deployed PostgreSQL database sitting at head `0011` runs `alembic upgrade head` and executes only `0012`–`0014`. Here, the "before" state is not transient — it is the real, currently-running production (or production-like) schema, with real rows in every table, accumulated over however long the deployment has been live. This is the path where `0012`'s `AT TIME ZONE 'UTC'` conversion actually operates on data that existed before the migration ran, where `0013`'s FK constraint replacement must not fail if any row-level state is unexpected, and where `0014`'s renames must find the *actual* current names in the catalog — which, unlike a fresh install, cannot be assumed to be pristine defaults if any out-of-band administrative change was ever made to the database directly (rare, but not proven impossible — see §7's identical caveat for `PMSchedule`/`CalibrationSchedule`).

**Why both paths must converge to the identical final schema, and how this proposal ensures it:**
- **Timezone (Migration A):** both paths end with the same five columns as `timestamptz`. On Path 1, the columns are naive for a few migration-steps and then converted; on Path 2, they have been naive for the database's entire real lifetime and are converted once. The `USING ... AT TIME ZONE 'UTC'` expression (§3.4) produces the same output type and the same value-preserving semantics regardless of which path produced the "before" naive value, so both converge. `due_at` is excluded from conversion on *both* paths identically (§3.4a) — there is no path-dependent special case for it.
- **FK policy (Migration B):** both paths end with all 25 FKs carrying an explicit `ondelete` policy in the PostgreSQL catalog (`pg_constraint.confdeltype`) **and** in the ORM `ForeignKey(ondelete=...)` declarations (§4 cross-check) — the migration changes the database, the accompanying model-source change changes what a fresh `Base.metadata.create_all()` would produce. Because Migration B updates both simultaneously, there is no window where Path 1 and Path 2 could disagree about the final `ondelete` value.
- **Index naming (Migration C):** this is the path where convergence is least automatic and needs the most explicit catalog inspection — see the dedicated §6.4 this round adds, which covers exactly this convergence question for index/constraint names specifically (ORM metadata, migration-only objects, and the 0014 rename all have to agree).

**Catalog-state inspection requirement, revised this round to a three-way check, not binary fail-closed:** none of Migrations A/B/C may assume the "before" name, type, or constraint state their `upgrade()` logic expects — each must query `information_schema.columns` (types), `pg_constraint` (FK/CHECK definitions and current names), or `pg_indexes` (index names) for the *actual* current catalog state immediately before acting. The prior round of this section stopped at "fail closed if that state does not match what was expected," which is necessary but incomplete: it only distinguished *matches-before-state* from *unexpected*, and did not account for the case this round's Path 1 correction identifies — *already-matches-target-state*, which is not an error, it is success achieved by a different route. See §8.7a immediately below for the full three-way policy this requires. This is still the same underlying discipline migration `0011`'s `_ensure_index_concurrently()` and migration `0005`'s `_preflight()` established (§8.1, §6.3) — catalog inspection before acting — just with a third outcome added.

### 8.7a Verify-and-no-op policy (new this round — resolves the primary blocker above)
...
2. **Already at target state** — the object already matches what the migration exists to produce (`timestamptz`; explicit `RESTRICT`/`SET NULL`; the target `ix_`/`uq_` name). This is the case §8.7's Path 1 correction identifies as newly possible: once the accompanying ORM model changes land, a fresh `0001`-driven install reaches this state without any of `0012`–`0014` doing anything. **The migration performs no schema rewrite and no destructive action — it verifies the state matches the target, logs that it found nothing to do, and exits successfully.** This is an intentional, expected no-op, not a degraded or partial run.
3. **Neither** — the object is in some other, unrecognized state (wrong type entirely; an unexpected constraint definition; a name matching neither the old nor the new convention). This is the only outcome that raises a clear `RuntimeError` naming the specific mismatch — genuinely unexpected state, not merely "already done."

**Concretely, per migration:**

- **Migration A (timezone):** for each of the five converting columns, query `information_schema.columns.data_type`. `timestamp without time zone` → transform (outcome 1). `timestamp with time zone` → no-op (outcome 2) — **and critically, do NOT run the `AT TIME ZONE 'UTC'` `ALTER ... USING` expression against a column already in this state**: applying that conversion a second time, to a value that is already correctly aware, does not raise an error — it reinterprets an already-correct instant as if it were an unlabeled one, producing a silently wrong result (an offset-shifted timestamp). This is the single most important reason this section exists: the risk here is not a crash, it is quiet data corruption on exactly the fresh-install path this proposal's own ORM changes newly make possible. Any other type → outcome 3, fail closed.
- **Migration B (FK policy):** for each of the 25 FKs, query `pg_constraint.confdeltype`. `a` (`NO ACTION`) → transform (drop/recreate with the target `ondelete`). Already matching the target (`r` for `RESTRICT`, `n` for `SET NULL` on the §9.1-approved subset) → no-op — recreating an identical constraint is not harmful here (unlike Migration A's case) but is still unnecessary churn and an avoidable brief `ACCESS EXCLUSIVE` lock, so it is skipped. Any other `confdeltype` (`c` `CASCADE`, `d` `SET DEFAULT`) → outcome 3, fail closed, since nothing in this proposal ever intends either.
- **Migration C (index naming):** for each of the 12 renames, query `pg_indexes`/`pg_constraint` for the **old** name first. Found → transform (rename). Not found → query for the **target** name — found → no-op (outcome 2; this is the fresh-install-after-ORM-fix case for the 7 auto-named constraints described in §6.4). Neither old nor target name found → outcome 3, fail closed (the object may have been renamed to something else entirely, or dropped, by an out-of-band change).

**Why this is a genuine convergence guarantee, not just error-avoidance:** with the three-way check, Migrations A/B/C produce the identical end state whether they run against a database that needed transformation (Path 2, or a Path 1 install predating the ORM fix) or a database that already had the target state (Path 1, post-ORM-fix) — the migration's *observable effect on the schema* is the same (target state reached) even though its *internal action* differed (transform vs. no-op). This is the concrete form of "both paths converge" that §8.7 asserts in the abstract.
...
| **ORM metadata drift (new this round — the primary blocker raised in this review round)** | `0001_initial.py` builds fresh databases from **current** ORM metadata, not a frozen historical snapshot. Once this proposal's required ORM changes land (§3.5's `DateTime(timezone=True)`, §4's `ForeignKey(ondelete=...)`, §6.4's explicit constraint names), a fresh install already reaches the target schema *before* Migrations A/B/C ever run. If those migrations assumed "the before state always exists" and blindly applied their transformation, Migration A's `AT TIME ZONE 'UTC'` conversion would silently **corrupt already-correct data** on that path — reinterpreting an already-aware, correct instant as if it were unlabeled, producing a wrong, offset-shifted value with no error raised. This is the most severe risk in this document: silent data corruption, not a crash. | The §8.7a verify-and-no-op policy: every migration checks actual catalog state first and classifies it as needs-transformation / already-at-target / neither, only transforming in the first case, no-op-and-succeed in the second, and fail-closed only in the third. Migration A specifically must never run its `USING ... AT TIME ZONE 'UTC'` expression against a column already `timestamptz`. |
...
*No production code was written or modified to produce this document. No migration was generated. No application file was modified. Every schema/database claim in this document was verified against a live PostgreSQL 16 instance rehearsed from `base` to `head` (migrations `0001`–`0011`) on a disposable scratch database created and dropped solely for this investigation — not assumed, and not read from a stale local test database. This document itself is committed and published as Draft PR #52 for independent review. FK policy, timezone approach (Option A), and the three-migration split are approved; implementation remains blocked pending sign-off on the remaining open items in §9 and the revisions in this round (historical `due_at` handling, catalog-aware migration strategy, index-naming convergence).*
