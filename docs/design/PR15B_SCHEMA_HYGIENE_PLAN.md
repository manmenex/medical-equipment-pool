...
**Status:** Design only, committed and published for independent architecture review as Draft PR #52 (`docs/pr15b-schema-hygiene-design-review`, documentation-only — no production code, no migrations). **Architecture decisions approved to date:** FK policy = `RESTRICT` (§4); Timezone = Option A (§3.5); Migration split = three migrations numbered `0012`–`0014` (§8.2). Implementation remains blocked pending full review sign-off — see the open items still tracked in §9.
...
**Path 1 — fresh database.** A brand-new PostgreSQL instance runs `alembic upgrade head`, which executes every migration from `0001` through `0014` (assuming Migrations A/B/C land as `0012`/`0013`/`0014`) in sequence, starting from nothing. Migration `0001_initial.py` bootstraps every table via `Base.metadata.create_all(bind=bind)` — meaning the *starting point* for a fresh install is whatever the current ORM model declarations say, not a fixed historical snapshot. Migrations `0002`–`0011` then apply their own changes on top, and finally `0012`–`0014` apply this proposal's timezone/FK/index changes. On a fresh database, every naive column, every implicit-`NO ACTION` FK, and every auto-named index/constraint exists only *transiently*, for the instant between `0001` completing and the relevant one of `0012`/`0013`/`0014` running — an observer who only ever looks at a fully-migrated fresh database never sees the "before" state at all.
...
**Catalog-state inspection requirement, stated once for all three migrations:** none of Migrations A/B/C may assume the "before" name, type, or constraint state their `upgrade()` logic expects — each must query `information_schema.columns` (types), `pg_constraint` (FK/CHECK definitions and current names), or `pg_indexes` (index names) for the *actual* current catalog state immediately before acting, and fail closed (raise a clear `RuntimeError` naming the mismatch) if that state does not match what the migration was written assuming. This is the same discipline migration `0011`'s `_ensure_index_concurrently()` and migration `0005`'s `_preflight()` already established (§8.1, §6.3) — §8.7 makes explicit that this applies to *all three* of PR15B's migrations, not only the index rename.
