...
status references predating that PR, found by a full six-file re-sweep
— see `docs/DECISION_LOG.md`'s two PR #134 entries for the exact
before/after text of each), merged on top of
...
**zero reviews**, zero comments, and zero review threads, with CI green
(6/6) and the reviewed head's tree independently verified
byte-identical to the merged squash commit's tree, sole parent
`84144f0...` confirmed. Per this repository's standing process, **no
separate "baseline adoption" PR is created** — the squash SHA became
authoritative immediately upon merge, recorded here by this PR (the
next PR that legitimately touches these governance files). **PR24D
(CI/CD & Staging) code/tooling is merged and complete. Operational
Staging evidence remains pending:** the manual `cd-staging.yml`
...
cloud infrastructure.** The Owner explicitly approved proceeding with a
zero-cost LOCAL execution of the existing Staging/UAT environment class
using Docker on a Windows PC, reachable by other authorized devices on
the same LAN, plus building an installer/deployment mechanism for it
(Setup.exe explicitly deferred until this script-based installer engine
is proven). **This is NOT a fourth environment** — OD-PR24-4's
taxonomy (Development, Staging/UAT, Production) is unchanged; local
execution is a hosting/execution method for the existing Staging/UAT
class only. **PR24D-L1 (Local Docker Staging/UAT Foundation) is now in
progress**: `deployment/local-staging/compose.yml` (PostgreSQL, Redis,
backend, frontend; PostgreSQL/Redis never exposed to the LAN; exactly
one backend replica/Uvicorn worker; no committed secrets, default
credentials, or demo data) plus a `COOKIE_SECURE` backend setting
(`backend/app/core/config.py`) decoupling the refresh-token cookie's
`Secure` attribute from `ENVIRONMENT` — real Production is unaffected;
only this local execution mode explicitly opts out, since it runs
`ENVIRONMENT=production` (to keep `validate_production_secrets()`'s
hardened checks) over plain HTTP on a trusted LAN, where a `Secure`
cookie would be silently dropped by the browser. See
`docs/design/PR24_PRODUCTION_DEPLOYMENT_GO_LIVE_PLAN.md` §32 for the
full architecture and the planned PR24D-L2 (installer/operations
scripts)/PR24D-L3 (local backup/restore rehearsal + runbook + final
governance sync) split. Local execution of this mechanism does **not**
constitute real managed-Staging evidence, a real managed-Staging
backup/restore rehearsal, or grounds to start PR24E — see the
"Operational Staging evidence remains pending" sentence above, which
this local-execution work does not change.
...
**The PR23 Owner Decision Closure round is also merged** (GitHub PR
#123, squash SHA `22ec7a25d686b0cd37d2a366172cb31a49eebff8`,
historical): all six PR23 Owner Decisions (OD-PR23-1 through OD-PR23-6
...
persisted evidence model) are Owner-approved per Recommendation, with
an explicit Owner clarification for OD-PR23-5's Pilot Ward selection/
duration/exit-criteria rules (see `docs/design/PR23_CUTOVER_READINESS_
PLAN.md` §26). That closure round released the fail-closed PR23B+
implementation-authorization gate (§27). **PR23B (Cutover Readiness
Evidence Foundation) is also merged** (GitHub PR #124, squash SHA
...
API) implementing OD-PR23-6's persisted-evidence approach, hardened by
...
decision logic, frontend, pilot execution, cutover execution, rollback
...
Gates A-F (§12) BLOCKER/WARNING/INFO (§13) evaluation endpoint against a
completed run's persisted evidence; no mutation, no persisted
gate/decision model, no Go/No-Go decision (Gate G remains PR23D's own
scope), hardened by one fix round (Gate B dataset-type check, both at
the evaluator and the PR23B completion boundary). **PR23D (Go/No-Go
Decision + Current-State Re-Issue Support) is also merged** (GitHub PR
#126, squash SHA `2da80231d4f037136b291863e379e739aa2905dd`,
historical): an immutable `CutoverGoNoGoDecision` evidence record (Gate
G), backed by migration `0022_cutover_go_no_go_decision`, requiring a
fresh Gates A-F re-evaluation at decision time; `GO` requires zero live
BLOCKER findings and full WARNING-code acknowledgement, `NO_GO` never
requires readiness success; no current-state re-issue write endpoint
was added (the existing `POST /borrow` Issue workflow already covers
...
operator UI (run list/detail, Gates A-F presentation, Go/No-Go decision
workflow) over the already-merged PR23B-D backend, only reading and
rendering backend-computed readiness state; no new backend route, no
migration. **PR23F (Cutover Runbook + Final Governance Close-out) is
also merged** (GitHub PR #128, squash SHA
`f35fe716d57c51042d86a661657f679799b6a9e3`, historical, superseded
several times since — see "Current baseline" above for the actual
current baseline) — a documentation-only operational runbook
...
complete.** Real Pilot execution, Production cutover, AppSheet's actual
read-only transition, a selected production deployment target, and a
rehearsed backup/restore procedure have **not** occurred. This baseline
entry will be updated again once the next governance-touching PR
merges.
...
#122), then `22ec7a25...` (GitHub PR #123), and now by this baseline
(`833f6758...`, GitHub PR #124). With
...
**Roadmap PR23 has since also fully completed** — PR23A (Architecture &
Operational Design), all six PR23 Owner Decisions (OD-PR23-1 through
OD-PR23-6) via the PR23 Owner Decision Closure round, PR23B (Cutover
Readiness Evidence Foundation), PR23C (Readiness Gate Evaluation), PR23D
(Go/No-Go Decision + Current-State Re-Issue Support), PR23E (Frontend /
Operator Workflow), and PR23F (Cutover Runbook + Final Governance
Close-out) are all merged; **Roadmap PR23 overall is now fully
implementation-complete**; its row has likewise moved off this table,
and PR24 is now shown with sequence "Current" in the table above. **The
Production Deployment & Go-Live Architecture Planning round has since
merged too** (GitHub PR #129, squash SHA
`599478992de363e1eda2fe8005ff79d565dee76d`, historical; includes
Fix Round 1's §15A liveness/readiness contract), **and all six Owner
Decisions it raised (OD-PR24-1 through OD-PR24-6,
`docs/design/PR24_PRODUCTION_DEPLOYMENT_GO_LIVE_PLAN.md` §28) are
Owner-approved via the PR24 Owner Decision Closure round, itself merged**
(GitHub PR #130, squash SHA `f64f7d148ba956adef43c5d363ad52680398541c`,
historical) — **PR24B (Deployment Foundation) has since merged too**
(GitHub PR #131, squash SHA
`d4a40349f62d76d129dcc6f1feea3e7e8fc8f28d`, historical) — **PR24C
(Backup & Restore) has since merged too** (GitHub PR #132, squash SHA
...
current baseline) — operational Staging evidence (manual workflow
execution, provider selection, real Staging infrastructure, real
backup/restore rehearsal) remains pending; PR24E through PR24G not
started.
PR22 through PR24's ordering is preserved as-is because it reflects an
...
now complete; PR24 depends on PR19–PR23, now satisfied), not a new
sequencing decision introduced by this or any prior governance sync.
...
record. **Roadmap PR23 has since also fully completed:** its first
slice, PR23A, merged (`docs/design/PR23_CUTOVER_READINESS_PLAN.md`,
GitHub PR #122), all six PR23 Owner Decisions are Owner-approved via
the PR23 Owner Decision Closure round (GitHub PR #123), PR23B (Cutover
Readiness Evidence Foundation) merged (GitHub PR #124), PR23C
(Readiness Gate Evaluation) merged (GitHub PR #125), PR23D (Go/No-Go
Decision + Current-State Re-Issue Support) merged (GitHub PR #126),
PR23E (Frontend / Operator Workflow) merged (GitHub PR #127), and
PR23F (Cutover Runbook + Final Governance Close-out) merged (GitHub PR
#128) — **Roadmap PR23 overall is now fully implementation-complete.**
This table now begins with PR24 — shown as the current Roadmap item.
**The Production Deployment & Go-Live Architecture Planning round has
since merged** (GitHub PR #129, squash SHA
`599478992de363e1eda2fe8005ff79d565dee76d`, historical;
`docs/design/PR24_PRODUCTION_DEPLOYMENT_GO_LIVE_PLAN.md`, including Fix
Round 1's §15A liveness/readiness contract), **and all six Owner
Decisions it raised are Owner-approved via the PR24 Owner Decision
Closure round, itself merged** (GitHub PR #130, squash SHA
`f64f7d148ba956adef43c5d363ad52680398541c`, historical, §28) —
...
`84144f096aacb9e2687422c7cd84cc1354346aa7`, historical — superseded by
GitHub PR #134's governance close-out, current baseline
`7e2bfb2001642ea9a9754310b85d1911b7b2be5c`, see "Current baseline"
above) — operational managed-Staging evidence remains pending. PR19B's
own historical relative-ordering note is preserved above.
