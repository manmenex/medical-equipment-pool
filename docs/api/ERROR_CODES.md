...
| `WEAK_PASSWORD` | 400 | `POST /auth/change-password`: the new password is shorter than the minimum length (6 characters), longer than bcrypt can hash (72 **bytes** — note Thai characters are 3 bytes each in UTF-8, so ~24 Thai characters), or starts/ends with whitespace |
...
| `CUTOVER_READINESS_RUN_NOT_FOUND` | 404 | `CutoverReadinessRunNotFoundError` | Roadmap PR23B: `GET /cutover-readiness-runs/{run_id}` or `POST .../complete` found no matching `CutoverReadinessRun` row |
| `CUTOVER_READINESS_RUN_NOT_MUTABLE` | 409 | `CutoverReadinessRunNotMutableError` | Roadmap PR23B: `POST .../complete` was called for a run whose `status` is not `pending`/`running` (already `completed` or `failed`). A completed run's evidence snapshot is permanently immutable -- the caller must create a new run (`supersedes_run_id`) instead |
| `CUTOVER_READINESS_RUN_VERSION_CONFLICT` | 409 | `CutoverReadinessRunVersionConflictError` | Roadmap PR23B: `POST .../complete`'s CAS `UPDATE ... WHERE id=:id AND version=:expected_version` matched zero rows -- a concurrent completion attempt already succeeded, or the caller's `expected_version` is stale. The caller must re-fetch the run and retry with the fresh version |
| `CUTOVER_READINESS_EVIDENCE_INVALID` | 422 | `CutoverReadinessEvidenceInvalidError` | Roadmap PR23B (hardened by Fix Round 1, and by PR23C Fix Round 1): one of the evidence references supplied to `POST .../complete` fails validation -- the referenced `ImportSource`/`LegacyMigrationAuthority`/`LegacyMigrationAuthorityCoverage`/`LegacyReconciliationRun`/`LegacyReconciliationSignOff`/`User`/`Ward` row does not exist, the supplied sign-off does not belong to the supplied reconciliation run, the supplied coverage does not belong to the supplied migration authority, the supplied reconciliation run is not bound to the supplied coverage, the supplied `equipment_master_import_source_id`'s owning `ImportSession.dataset_type` is not the Equipment Master dataset type (PR23C Fix Round 1 -- a field's name is not type safety), or `cutover_instant` is earlier than the bound coverage artifact's `live_system_start` (design §9). The whole `MigrationAuthority -> Coverage -> ReconciliationRun -> SignOff` provenance chain, and the Equipment Master evidence's own dataset identity, are validated fresh inside the same transaction as the completion `UPDATE`, never trusted from client input alone -- existence of each individual row is insufficient on its own |
| `CUTOVER_READINESS_DATABASE_MIGRATION_HEAD_UNAVAILABLE` | 503 | `CutoverReadinessDatabaseMigrationHeadUnavailableError` | Roadmap PR23B Fix Round 1 (reused by PR23C's Gate A freshness re-check): could not establish the database's own current Alembic revision by querying `alembic_version` (the table/row does not exist, or more than one row exists). `database_migration_head` is never accepted from client/API-caller input -- it must prove the actual schema state, so it is always read server-side. No readiness run is created (PR23B), and no gate evaluation is returned (PR23C), when this is raised |
| `CUTOVER_READINESS_GATE_EVALUATION_REQUIRES_COMPLETED_RUN` | 422 | `CutoverReadinessGateEvaluationRequiresCompletedRunError` | Roadmap PR23C: `GET .../gate-evaluation` was called for a `CutoverReadinessRun` whose `status` is not `completed` -- a `pending`/`running`/`failed` run has no persisted evidence snapshot (every evidence-reference column is still `NULL`), so gate evaluation is meaningless until the run reaches `completed` |
| `CUTOVER_DECISION_REQUIRES_COMPLETED_RUN` | 422 | `CutoverDecisionRequiresCompletedRunError` | Roadmap PR23D: `POST .../decision` was called for a `CutoverReadinessRun` whose `status` is not `completed` -- a Go/No-Go decision requires a fully captured evidence snapshot |
| `CUTOVER_DECISION_RUN_SUPERSEDED` | 409 | `CutoverDecisionRunSupersededError` | Roadmap PR23D (design §16): the readiness run has since been superseded by a newer run (some other `CutoverReadinessRun.supersedes_run_id` points back at it) -- a final Gate G decision is only ever recorded against the current, non-superseded readiness run |
| `CUTOVER_DECISION_STALE_VERSION` | 409 | `CutoverDecisionStaleVersionError` | Roadmap PR23D: the caller's `expected_version` no longer matches the `CutoverReadinessRun`'s current `version`, checked under the run's own lock -- re-fetch the run and retry with its current version |
| `CUTOVER_DECISION_ALREADY_EXISTS` | 409 | `CutoverDecisionAlreadyExistsError` | Roadmap PR23D: `POST .../decision` was called for a run that already has a persisted `CutoverGoNoGoDecision` row (`UNIQUE(cutover_readiness_run_id)`) -- a decision is never created twice or modified; `GET .../decision` reads the existing, immutable decision |
| `CUTOVER_DECISION_BLOCKED_BY_READINESS` | 422 | `CutoverDecisionBlockedByReadinessError` | Roadmap PR23D (design §12/§13): a `GO` decision was attempted while a fresh re-evaluation of Gates A-F still returns at least one BLOCKER item -- cutover Go is deterministically impossible while any mandatory gate fails; `NO_GO` is never blocked by this check |
| `CUTOVER_DECISION_WARNINGS_NOT_ACKNOWLEDGED` | 422 | `CutoverDecisionWarningsNotAcknowledgedError` | Roadmap PR23D (design §13): a `GO` decision was attempted without acknowledging every currently-live WARNING item code from a fresh Gate A-F re-evaluation -- compared against the live warning-code set computed at decision time, never an earlier/stale evaluation; only raised for `GO` |
| `CUTOVER_DECISION_NOT_FOUND` | 404 | `CutoverDecisionNotFoundError` | Roadmap PR23D: `GET .../decision` was called for a `CutoverReadinessRun` that has no persisted `CutoverGoNoGoDecision` row yet -- distinct from `CUTOVER_READINESS_RUN_NOT_FOUND` (the run itself may exist and simply not have a final decision recorded) |
