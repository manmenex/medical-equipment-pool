...
| `CUTOVER_READINESS_EVIDENCE_INVALID` | 422 | `CutoverReadinessEvidenceInvalidError` | Roadmap PR23B: one of the evidence references supplied to `POST .../complete` fails validation -- the referenced `ImportSource`/`LegacyMigrationAuthority`/`LegacyMigrationAuthorityCoverage`/`LegacyReconciliationRun`/`LegacyReconciliationSignOff`/`User`/`Ward` row does not exist, the supplied sign-off does not belong to the supplied reconciliation run, or `cutover_instant` is earlier than the bound coverage artifact's `live_system_start` (design §9). Every reference is validated fresh inside the same transaction as the completion `UPDATE`, never trusted from client input alone |
