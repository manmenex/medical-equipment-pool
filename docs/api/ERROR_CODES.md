...
| `IMPORT_DRY_RUN_PLAN_NOT_FOUND` | 404 | `ImportDryRunPlanNotFoundError` | Roadmap PR20D (`docs/design/PR20_EQUIPMENT_MASTER_IMPORT_PLAN.md` §14.6, §21): no plan matches the requested lookup -- either the session has never had a successful dry-run (`GET /import-sessions/{id}/dry-run-plan`, no `active` plan exists), or `{plan_id}` in `POST .../dry-run-plan/{plan_id}/confirm` doesn't reference a plan belonging to this session at all (ownership check). Never leaks a plan id belonging to a different session |
| `IMPORT_DRY_RUN_PLAN_STALE` | 409 | `ImportDryRunPlanStaleError` | Roadmap PR20D (design §14.4a), fix round 2 (PR94-H1/M2): `POST /import-sessions/{id}/dry-run-plan/{plan_id}/confirm` found the plan but it is no longer `active` (superseded by a newer dry-run, or already `consumed`/`failed`). Distinct from `IMPORT_SESSION_INVALID_STATE`, which is raised instead when the *plan* is still `active` but the *owning session* has itself moved out of `dry_run_completed` (e.g. a concurrent `cancel` won the race) -- the two conditions are never conflated into one code. The operator must re-fetch `GET .../dry-run-plan` and review the current plan before confirming again -- never silently retried against the same stale `plan_id` |
