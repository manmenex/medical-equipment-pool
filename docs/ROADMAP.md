...
`7e2bfb2001642ea9a9754310b85d1911b7b2be5c`** — the real squash-merge SHA
of GitHub PR **#134**, "PR24D — Post-Merge Governance Close-out" (folded
in PR #133's real squash SHA across governance authority docs; a Fix
Round 1 then closed four older, pre-existing stale current-baseline/
status references found by a full six-file re-sweep — see
`docs/DECISION_LOG.md` for the exact before/after text of each),
squash-merged into `claude/medical-equipment-pool-0c7fz0` on top of
`84144f096aacb9e2687422c7cd84cc1354346aa7` (GitHub PR #133, PR24D — now
historical/superseded by this baseline). PR #134's final reviewed
feature-branch head (`32998db1794e479a676f5d6cdaa7ac248f9a8b80`) carried
**zero reviews**, zero comments, and zero review threads, CI green 6/6,
and the reviewed head's tree was verified byte-identical to the merged
squash commit's tree, sole parent `84144f096aacb9e2687422c7cd84cc1354346aa7`
confirmed. Per this repository's standing process, **no separate
self-referential "baseline adoption" PR is created for PR #134's squash
SHA** — it became authoritative immediately upon merge, and its
recording here is folded into this PR (the next PR that legitimately
touches these governance files). **PR24D (CI/CD & Staging) code/tooling
is merged and complete. Operational Staging evidence remains pending:**
...
rehearsal against a managed-Staging target has not yet been performed
(see `docs/runbooks/PR24_STAGING_DEPLOYMENT_RUNBOOK.md`). OD-PR24-1
remains resolved at the architecture-class level (Managed Application
Platform + Managed PostgreSQL) — the specific provider is an
...
its start gated on real managed-Staging availability and sufficient
operational deployment/backup evidence; PR24F (Pilot Execution) and
PR24G (Production Go-Live Governance) also remain not started. **PR24
overall is in progress, not complete.** Real Pilot execution, Production
cutover, AppSheet's actual read-only transition, and a selected
commercial provider have **not** occurred.
...
an installer/deployment mechanism for it (Setup.exe deferred until a
...
Foundation) is now in progress**: `deployment/local-staging/compose.yml`
(PostgreSQL/Redis never LAN-exposed, exactly one backend replica/
Uvicorn worker, no committed secrets/default credentials/demo data)
plus a `COOKIE_SECURE` backend setting decoupling the refresh-token
cookie's `Secure` attribute from `ENVIRONMENT` for this local mode only
— see `docs/design/PR24_PRODUCTION_DEPLOYMENT_GO_LIVE_PLAN.md` §32 for
the full architecture, the planned PR24D-L2/L3 split, and the binding
...
CI/CD & Staging) is now historical/superseded, per the paragraph
above.

`cd9764ef5ba5e56062ee41266c8d96e50f1152c0` (GitHub PR #132, PR24C —
Backup & Restore) is now historical/superseded, per the paragraph
above.

`d4a40349f62d76d129dcc6f1feea3e7e8fc8f28d` (GitHub PR #131, PR24B —
Deployment Foundation) is now historical/superseded, per the
paragraph above.
...
`2da80231d4f037136b291863e379e739aa2905dd` (GitHub PR #126, PR23D — Go/
No-Go Decision + Current-State Re-Issue Support, including Fix Round 1)
is now historical/superseded.

`c10f5082fdc5cb7fd66615fe25516a4982297026` (GitHub PR #125, PR23C —
Readiness Gate Evaluation, including Fix Round 1) is now
historical/superseded.

`833f6758a93a78398207d64fbefa65ff2802cf46` (GitHub PR #124, PR23B —
Cutover Readiness Evidence Foundation) is now historical/superseded.

`22ec7a25d686b0cd37d2a366172cb31a49eebff8` (GitHub PR #123, PR23 Owner
Decision Closure) is now historical/superseded.
...
`833f6758a93a78398207d64fbefa65ff2802cf46`, historical — see the top of
this document): an additive backend-only persistence foundation
...
own scope), hardened by one fix round (Fix Round 1) correcting a P1
evidence-integrity finding — Gate B's evaluator and the PR23B completion
boundary both accepted an `ImportSource` whose owning `ImportSession.
dataset_type` was not actually `equipment_master`, so a cross-wired
`legacy_transaction_history` source could pass as Equipment Master
evidence; both layers now reject `GATE_B_WRONG_DATASET_TYPE` — see
`docs/design/PR23_CUTOVER_READINESS_PLAN.md` §27 for the precise scope
boundary and the Fix Round 1 entry. **PR23D (Go/No-Go Decision +
Current-State Re-Issue Support) is also merged** (GitHub PR #126, squash
SHA `2da80231d4f037136b291863e379e739aa2905dd`, historical — see the
top of this document): an immutable `CutoverGoNoGoDecision` evidence
record (Gate G, §12), backed by migration `0022_cutover_go_no_go_decision`,
recording a fresh Gates A-F re-evaluation at decision time (never a
cached/client-supplied evaluation): `GO` requires zero live BLOCKER
findings and full acknowledgement of every live WARNING code; `NO_GO`
never requires readiness success. No current-state re-issue write
endpoint was added — the existing `POST /borrow` Issue workflow already
satisfies the re-issue requirement (manual, per-equipment, duplicate-OPEN
protected), hardened by one fix round correcting a whole-schema
`ON DELETE RESTRICT` foreign-key-count regression test that migration
`0022` had not yet been rolled into — see
`docs/design/PR23_CUTOVER_READINESS_PLAN.md` §27 for the precise scope
boundary. **PR23E (Frontend / Operator Workflow) is also merged**
(GitHub PR #127, squash SHA
`8644536403eeec269e6dadf835f1bda3844b6cce`, historical — see the
top of this document): the operator-facing Thai-first UI over the
merged PR23B-D backend (run list/detail, Gates A-F presentation, the
Go/No-Go decision workflow) that only reads and renders
backend-computed readiness state, never recalculating readiness
client-side — zero backend/migration change. **PR23F (Cutover Runbook +
Final Governance Close-out) is also merged** (GitHub PR #128, squash
SHA `f35fe716d57c51042d86a661657f679799b6a9e3`, now
historical/superseded — see the top of this document): a
documentation-only operational runbook (`docs/runbooks/
PR23_CUTOVER_RUNBOOK.md` — T0-T4 procedure, rollback, contact/evidence
templates) plus the final governance close-out recording Roadmap PR23
implementation as complete; real Pilot/Production execution remains
...
`f64f7d148ba956adef43c5d363ad52680398541c`, now historical/superseded —
see the top of this document), recording all six Owner Decisions this
design raised (OD-PR24-1 through OD-PR24-6) as Owner-approved
...
network access; RPO ≤ 1 hour / RTO ≤ 4 hours / 30-day retention as
backup targets (not yet rehearsed); three environments (Development,
...
Owner. **PR24B (Deployment Foundation) is also merged** (GitHub PR
#131, squash SHA `d4a40349f62d76d129dcc6f1feea3e7e8fc8f28d`, historical):
the fail-closed readiness endpoint (`GET /api/v1/ready`), the
production-safe admin bootstrap script (`app.scripts.bootstrap_admin`),
the scheduler single-instance deployment invariant
(`backend/Dockerfile` `--workers 1`, `docker-compose.prod.yml`
`replicas: 1`), and fail-closed production configuration checks
(default `JWT_SECRET_KEY`/`DATABASE_URL`/`ALLOWED_ORIGINS` in
production, hardened across two independent-review fix rounds) — no
infrastructure provisioning, no commercial provider selected, no
Pilot/Production traffic served. **PR24C (Backup & Restore) is also
merged** (GitHub PR #132, squash SHA
`cd9764ef5ba5e56062ee41266c8d96e50f1152c0`, historical — superseded by
PR24D, see the top of this document): `backend/scripts/backup_postgres.py` /
`restore_postgres.py` / `prune_backups.py` (logical `pg_dump`/
`pg_restore` backup, checksum verification, a hard restore-target guard
refusing any Production-labeled or source-identical target, unconditional
since Fix Round 1, 30-day retention cleanup), proven via a real round
trip against ephemeral CI PostgreSQL databases, plus the operator
runbook and rehearsal evidence template (`docs/runbooks/
PR24_BACKUP_RESTORE_RUNBOOK.md`) — this proved the tooling, not
operational readiness. **PR24D (CI/CD & Staging) is merged and complete**
(GitHub PR #133, squash SHA `84144f096aacb9e2687422c7cd84cc1354346aa7`,
current baseline — see the top of this document): the immutable-artifact
build/scan/migrate/deploy/verify mechanism
(`.github/workflows/cd-staging.yml`, `backend/scripts/deploy_migrate.py`,
`backend/scripts/staging_smoke_check.py`), including two independent-
review fix rounds (CRITICAL image scan hardened into a structural
pre-migration gate, mutable commit-SHA tags replaced by digest-pinned
immutable artifact identity; a `workflow_dispatch` input shell-injection
path closed via `env:` routing), proven only against an ephemeral,
CI-provisioned target — no hosting provider selected, no real Staging
infrastructure provisioned, no paid resource created, and the manual
`cd-staging.yml` workflow has not yet been executed even once (see
`docs/runbooks/PR24_STAGING_DEPLOYMENT_RUNBOOK.md`); a real
Staging-class backup/restore rehearsal remains deferred until real
Staging infrastructure exists. **PR24E (UAT Readiness) remains not
started**, gated on real Staging availability and sufficient operational
evidence; PR24F (Pilot Execution) and PR24G (Production Go-Live
Governance) also remain not started.
...
Readiness)'s first slice, PR23A, is merged; all six PR23 Owner
Decisions (OD-PR23-1 through OD-PR23-6) are Owner-approved via the PR23
Owner Decision Closure round, which is itself merged; PR23B (Cutover
Readiness Evidence Foundation) is merged; PR23C (Readiness Gate
Evaluation) is merged; PR23D (Go/No-Go Decision + Current-State
Re-Issue Support) is merged; PR23E (Frontend / Operator Workflow)
is merged; and PR23F (Cutover Runbook + Final Governance Close-out)
is merged. **Roadmap PR23 overall is now fully implementation-
complete.** The Production Deployment & Go-Live Architecture Planning
round (`docs/design/PR24_PRODUCTION_DEPLOYMENT_GO_LIVE_PLAN.md`) is
the next planned item, preceding any Roadmap PR24 implementation
slice** (see the top of this document).
...
| PR23 | Cutover Readiness — **PR23A (Architecture & Operational Design) COMPLETE / MERGED** (`docs/design/PR23_CUTOVER_READINESS_PLAN.md`, GitHub #122, squash SHA `7ca9c87b4c525a1835403dac5d08e6e1be79d33b`, historical); **PR23 Owner Decision Closure COMPLETE / MERGED** (all six OD-PR23-1 through OD-PR23-6 Owner-approved, GitHub #123, squash SHA `22ec7a25d686b0cd37d2a366172cb31a49eebff8`, historical); **PR23B (Cutover Readiness Evidence Foundation) COMPLETE / MERGED** (GitHub #124, squash SHA `833f6758a93a78398207d64fbefa65ff2802cf46`, historical) — backend-only persistence foundation for OD-PR23-6; **PR23C (Readiness Gate Evaluation) COMPLETE / MERGED** (GitHub #125, squash SHA `c10f5082fdc5cb7fd66615fe25516a4982297026`, historical) — read-only Gates A-F BLOCKER/WARNING/INFO evaluation, includes Fix Round 1 (Gate B dataset-type hardening); **PR23D (Go/No-Go Decision + Current-State Re-Issue Support) COMPLETE / MERGED** (GitHub #126, squash SHA `2da80231d4f037136b291863e379e739aa2905dd`, historical) — Gate G immutable decision record; **PR23E (Frontend / Operator Workflow) COMPLETE / MERGED** (GitHub #127, squash SHA `8644536403eeec269e6dadf835f1bda3844b6cce`, historical) — Thai-first operator UI over the merged PR23B-D backend, no new backend route/migration; **PR23F (Cutover Runbook + Final Governance Close-out) COMPLETE / MERGED** (GitHub #128, squash SHA `f35fe716d57c51042d86a661657f679799b6a9e3`, historical — superseded by the current baseline at the top of this document) — documentation-only operational runbook + final governance close-out; **PR23 overall is now fully implementation-complete** |
| PR24 | Go-live / deployment — **Architecture & Go-Live Planning COMPLETE / MERGED** (`docs/design/PR24_PRODUCTION_DEPLOYMENT_GO_LIVE_PLAN.md`, GitHub #129, squash SHA `599478992de363e1eda2fe8005ff79d565dee76d`, historical; includes Fix Round 1's §15A liveness/readiness contract); **PR24 Owner Decision Closure COMPLETE / MERGED** (GitHub #130, squash SHA `f64f7d148ba956adef43c5d363ad52680398541c`, historical; all six OD-PR24-1 through OD-PR24-6 Owner-approved, §28 — Managed Application Platform + Managed PostgreSQL; public HTTPS + existing auth; RPO ≤1h/RTO ≤4h/30-day retention targets; three environments with Pilot inside Production; provider-supplied hostname accepted pre-Production, custom domain deferred as a resolved pre-Go-Live policy; project Owner as Primary Technical/Support/Incident Owner); **PR24B (Deployment Foundation) COMPLETE / MERGED** (GitHub #131, squash SHA `d4a40349f62d76d129dcc6f1feea3e7e8fc8f28d`, historical) — fail-closed readiness endpoint, production-safe admin bootstrap script, scheduler single-instance deployment invariant, fail-closed production config checks; no infrastructure provisioned, no provider selected; **PR24C (Backup & Restore) COMPLETE / MERGED** (GitHub #132, squash SHA `cd9764ef5ba5e56062ee41266c8d96e50f1152c0`, historical — superseded by PR24D, see the top of this document) — `pg_dump`/`pg_restore`/prune tooling, CI-proven round trip, operator runbook; no real Staging rehearsal yet; **PR24D (CI/CD & Staging) code/tooling COMPLETE / MERGED** (GitHub #133, squash SHA `84144f096aacb9e2687422c7cd84cc1354346aa7`, historical — superseded, see the top of this document) — immutable-artifact build/scan/migrate/deploy/verify mechanism including two independent-review fix rounds (image-scan hard-gating + digest-pinned identity; workflow_dispatch shell-injection fix), proven against an ephemeral CI-provisioned target only; manual workflow execution pending, no provider selected, no real Staging infrastructure provisioned, no real backup/restore rehearsal performed; **PR24D — Post-Merge Governance Close-out COMPLETE / MERGED** (GitHub #134, squash SHA `7e2bfb2001642ea9a9754310b85d1911b7b2be5c`, current baseline — see the top of this document); **PR24D-L1 (Local Docker Staging/UAT Foundation) IN PROGRESS** — Owner-approved zero-cost local execution of the existing Staging/UAT class via Docker on a LAN host, not a fourth environment (see §32 of the PR24 design doc); PR24E through PR24G not started; Pilot and Production not executed; **PR24 overall is not yet complete** |
