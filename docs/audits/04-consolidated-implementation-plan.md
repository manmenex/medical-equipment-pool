...
- **Actor versus subject:** `actor_user_id`/the audit row's user reference identifies the authenticated user who performed the action. The affected user or other entity is the subject and is identified separately by `entity_type`/`entity_id` and safe before/after data. Login failures have no actor; when an account is known it may be recorded only as the subject. Unknown submitted identifiers must never be stored raw or made enumerable; a one-way correlation hash is acceptable.
...
- **Test requirements:** Unit and API tests cover the canonical writer, actor/subject semantics, recursive redaction, identifier hashing, valid/invalid/oversized request IDs, bounded User-Agent/IP handling, read authorization and pagination, exactly-one-event coverage, mandatory atomic rollback, and login-failure persistence. PostgreSQL-backed tests must exercise transaction/savepoint behavior and the real Alembic upgrade → downgrade → upgrade path, including both fresh-database and pre-PR3 starting states.
- **Acceptance criteria:** All current in-scope events are attributable without confusing actor and subject; secrets and raw failed-login identifiers cannot reach audit storage; mandatory mutations and audit writes are atomic; failed logins remain auditable; request metadata is bounded and validated; audit reads are Administrator-only and bounded/deterministic; migration round trips pass on PostgreSQL; and every action produces exactly one event.
