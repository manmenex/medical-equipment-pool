...
Roadmap PR7 (both slices, GitHub PR #19/#20) remains the most recent *fully complete* Roadmap-numbered work; Roadmap PR8 is only partially complete (PR8A merged; PR8B in progress; PR8C not started). See `knowledge/adr/ADR-005-transaction-model.md`, `docs/DOMAIN_MODEL.md`, and `docs/DECISION_LOG.md` for PR7 and PR8A.

## Next planned PR

Roadmap PR8 (PR8B slice) — receipt outcome contract narrowing (`receipt_outcome`: `usable`/`defective`, replacing `condition`) — **in progress, Draft pending review** (no GitHub PR number or squash SHA yet; see `docs/DECISION_LOG.md` "Roadmap PR8 (PR8B slice)" and `knowledge/adr/ADR-006-receipt-outcome-contract.md`). This slice is backend-only: the frontend has not been updated to match (`docs/TECH_DEBT.md` TD-006 — the backend and a follow-up frontend change must be deployed together), and the distinguishable race-loss-vs-genuine-repeat error message is out of scope entirely, now named **Roadmap PR8 (PR8C slice)**, not started. PR8A (the database-level concurrency guard) is merged; concurrent-receipt protection is no longer an outstanding pilot blocker, but PR8's full scope is not complete until PR8A, PR8B, and PR8C all merge.

## Outstanding work

- Roadmap PR8 (PR8B slice, in progress; PR8C slice, not started) through PR15 (contract narrowing, race-vs-repeat distinction, ward correction, role consolidation, frontend terminology, inventory import, search/reporting, reliability/performance hardening, observability/schema hygiene) are planned or in progress, none merged yet.
