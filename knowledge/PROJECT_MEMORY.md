...
**Roadmap PR10 (Role Model Consolidation) is now in progress**, on a Draft PR from branch `feature/pr10-role-consolidation`. It replaces the legacy 5-role model with the confirmed 3-role model (`administrator`/`equipment_pool_staff`/`read_only`), including a new Alembic migration (`0009_role_consolidation.py`) with a fail-closed manifest mechanism for the three legacy roles that have no confirmed equivalent. See `docs/DECISION_LOG.md` ("Roadmap PR10") and `docs/BUSINESS_RULES.md` ("Roles and the confirmed 3-role permission matrix") for full detail. Not yet merged — the "Current baseline" above is unaffected until it does. Roadmap PR11 through PR15 (frontend terminology through observability/schema hygiene) remain planned and not yet started — see `docs/ROADMAP.md`. The original, pre-split PR8 design document (`docs/design/PR8_IMPLEMENTATION_PLAN.md`, uncommitted, design-only) is what PR8A implemented Option A from and what PR8C's race-vs-repeat distinction was already anticipated in (Section 6); it remains untracked.
