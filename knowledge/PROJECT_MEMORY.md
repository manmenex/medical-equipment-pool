...
squash SHA `f64f7d148ba956adef43c5d363ad52680398541c`, historical),
...
superseded by PR24D): `pg_dump`/`pg_restore`/prune tooling, checksum
verification, a hard Production-restore-target guard, 30-day retention
cleanup, proven via a real round trip against ephemeral CI PostgreSQL
...
digest-pinned artifact identity; `workflow_dispatch` shell-injection
fix), proven only against an ephemeral, CI-provisioned target.
...
yet been performed (see `docs/runbooks/
PR24_STAGING_DEPLOYMENT_RUNBOOK.md`). **PR24E (UAT Readiness) remains
not started**, gated on real Staging availability and sufficient
operational evidence; PR24F and PR24G also remain not started. **PR24
...
commercial provider have **not** occurred. See the "Roadmap PR22"
...
itself since superseded by `d64d50d...`, PR21E's own baseline at that
time, itself since superseded multiple times over; see "Current
baseline and Roadmap" above for the live value). GitHub PR #81, an earlier
...
PR #130, squash SHA `f64f7d148ba956adef43c5d363ad52680398541c`,
...
- PR24: Go-live / deployment. Dependency (PR19–PR23) satisfied.
  ...
  **PR24C (Backup & Restore) COMPLETE / MERGED** (GitHub PR #132,
  squash SHA `cd9764ef5ba5e56062ee41266c8d96e50f1152c0`, historical —
  superseded by PR24D) — `pg_dump`/`pg_restore`/prune tooling,
  ...
  provisioned, no real backup/restore rehearsal performed. **PR24E
  (UAT Readiness) NOT STARTED**, gated on real Staging availability
  and sufficient operational evidence; PR24F and PR24G also NOT
  STARTED.
