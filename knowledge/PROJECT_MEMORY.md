...
- Concurrent receipt is guarded at the database level: `app.crud.transaction.close()` decides the sole winner among racing receipt requests solely by the affected-rowcount of a single conditional `UPDATE ... WHERE id = :id AND status = 'open'`, never by an application-level pre-check. Every losing request is rolled back before any business side effect runs (no equipment-status change, no status-history row, no audit row) — only the winner performs side effects, in the same transaction. Proven with deterministic PostgreSQL tests forcing genuine contention across bursts of 1, 2, 5, 10, and 50 concurrent requests. Implemented as Roadmap PR8A (GitHub PR #26); see `docs/DECISION_LOG.md`.
