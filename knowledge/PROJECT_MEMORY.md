...
`53340f6d7d5c8cda685235411b60a57d2d033a7e` (branch `claude/medical-equipment-pool-0c7fz0`, squash merge of Roadmap PR10, Role Model Consolidation, GitHub PR #36), on top of Roadmap PR9's PR9B slice (frontend audited ward correction, GitHub PR #34, squash SHA `bfe8a42a55d738d3e591ce27145c7918186643ac`), which sits on top of Roadmap PR9A (backend audited ward correction, GitHub PR #33, squash SHA `9cef8411f067b14dd417d3dcd1335567cb669868`), which sits on top of the post-PR8-completion documentation follow-up (GitHub PR #32, squash SHA `94a14b8269b5e959d146caa2a3597f06fd02a3c0`) as of this snapshot. Always confirm against `knowledge/CONTEXT.md` and `docs/ROADMAP.md`, which are updated more frequently than this file.
...
**Roadmap PR10 (Role Model Consolidation) is now merged** (GitHub PR #36, squash `53340f6d7d5c8cda685235411b60a57d2d033a7e`), from branch `feature/pr10-role-consolidation`. It replaced the legacy 5-role model with the confirmed 3-role model (`administrator`/`equipment_pool_staff`/`read_only`), including a new Alembic migration (`0009_role_consolidation.py`) with a fail-closed manifest mechanism for the three legacy roles that had no confirmed equivalent. Three post-merge-request Codex review rounds hardened the migration's atomicity/audit/lossless-restoration/ownership guarantees before merge. See `docs/DECISION_LOG.md` ("Roadmap PR10") and `docs/BUSINESS_RULES.md` ("Roles and the confirmed 3-role permission matrix") for full detail. This documentation-only governance sync (mirroring the one after Roadmap PR9, GitHub PR #35) records PR10's completion and advances the "Current baseline" above. Roadmap PR11 (Frontend Terminology and Workflow UI Pass) is now the next planned item; PR12 through PR15 remain planned and not yet started — see `docs/ROADMAP.md`. The original, pre-split PR8 design document (`docs/design/PR8_IMPLEMENTATION_PLAN.md`, uncommitted, design-only) is what PR8A implemented Option A from and what PR8C's race-vs-repeat distinction was already anticipated in (Section 6); it remains untracked.
