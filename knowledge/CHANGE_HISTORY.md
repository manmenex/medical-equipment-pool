...
## Roadmap PR21 complete: PR21D1–PR21E merged, PR21F closes Roadmap PR21 (GitHub PR #107, #108, #109, #110)

Roadmap PR21 (Legacy Receive and Issue History Import) is now fully complete. Building on PR21-Foundation/PR21A/PR21B/PR21C and the SDC-exclusion decision (see the entry immediately below), the four remaining implementation slices merged:

- **PR21D1 — Combined Canonical Adapter + Source Admission** (GitHub PR #107, squash SHA `50b9e77269b238d95fb34b28d0bc223a369951e2`) — composed the merged PR21B (`issue.py`)/PR21C (`receive.py`) parsers into the production `legacy_transaction_history` `ImportAdapter` and registered it; added the authorized `PR21_MAX_UPLOAD_BYTES = 32 MiB` bounded upload allowance (generic 10 MiB cap unchanged for every other dataset); wired the mandatory all-or-nothing validation gate — one workbook → one `ImportSession` → one `ImportSource` → all four canonical sheets → one aggregate validation decision, an Issue-only or Receive-only `LegacyHistoryDryRunPlan` structurally impossible. Still no Issue↔Receive pairing, no SDC, no Equipment mutation.
- **PR21D2 — Historical Event Execution** (GitHub PR #108, squash SHA `c4788de06bed9a13aa5ec981fb8e19c67bc5720b`) — executes the validated, immutable `LegacyEquipmentEvent` INSERTs (`ISSUE`/`RECEIVE`, never `BorrowTransaction` replay, live dispatch/receipt, or `Equipment.status`/version/location/lifecycle mutation) from an admitted, confirmed `LegacyHistoryDryRunPlan`; reuses PR19/PR20's existing lease/fencing/recovery/audit machinery, idempotent and migration-authority-scoped.
- **PR21E0 — Legacy Import Operator API Surface** (GitHub PR #109, squash SHA `78eeea7827c53443f34de9e516573c2ed7c59581`) — closed the two remaining production-readiness gaps this design document itself had flagged as still-missing: an Administrator-facing `POST/GET /legacy-migration-authorities` API to explicitly approve a workbook checksum by exact scope (`pr21_legacy_transaction_history_v1`), fail-closed, no automatic approval and no delete/revoke workflow added; and a PR21-specific, statically typed `GET .../legacy-history/dry-run-plan[/{plan_id}/rows|/confirm]` HTTP surface, a deliberately separate route family from PR20's own `.../dry-run-plan` routes (byte/field/OpenAPI-unchanged for Equipment Master).
- **PR21E — Legacy History Frontend Real Integration** (GitHub PR #110, squash SHA `d64d50d09cdf8ed7ddc1f5116b38805dfcbc7810`) — replaced the PR19B mock Receive/Issue frontend with a real, single combined `legacy_transaction_history` operator workflow against PR21D1/D2/E0: create → upload → an explicit, never-auto-approved migration-authority approval step (checksum sourced only from the real upload response, never browser-computed or hand-typed) → validate → dry-run → paginated ISSUE/RECEIVE row review → confirm → execute, each mutating action behind its own confirmation dialog. Session-detail routing now dispatches on the session's own real `dataset_type` (fetched once), replacing the prior UUID-shape heuristic that would otherwise have silently routed every real PR21 session into the Equipment Master panel. `MockImportClient`, the fixture set, and the skeleton banner were removed entirely — no production PR21 path goes through mock data. **PR #110's independent Final Merge Gate recorded no finding of any kind** (zero review threads, zero comments) — not an accepted P2, a genuine absence of findings; CI green 6/6; reviewed head and merged squash tree verified byte-identical.

**Roadmap PR21 (Legacy Receive and Issue History Import) is now fully complete** — Equipment Master (PR20) and legacy transaction history (PR21) are both real, backend-and-frontend-integrated import workflows; `MockImportClient`/the PR19B skeleton no longer serve any production path. See `docs/DECISION_LOG.md` ("Roadmap PR21 complete: PR21D1–PR21F merged") for the full closure record. **At the time this entry was written, the next planned Roadmap item was PR22 — Legacy Data Validation and Reconciliation** (cross-import validation, reconciliation, source traceability verification, duplicate review, unified legacy/new history validation), depending on PR20 and PR21, both then complete. **Update (2026-08-24, this same historical paragraph): Roadmap PR22 has since progressed through architecture design, all seven Owner Decisions, and every implementation slice — PR22A-F implementation is now complete and merged; PR22G (governance close-out) is in progress, not yet merged; Roadmap PR22 overall remains open until PR22G itself merges. The next planned Roadmap item after PR22 closes is PR23 — Cutover Readiness, not started. See `docs/ROADMAP.md`, `docs/ROADMAP_STATUS.md`, and `docs/DECISION_LOG.md` for current status.** **Update (2026-08-25, this same historical paragraph): PR22G has since merged (GitHub PR #121) — Roadmap PR22 is now fully complete. Roadmap PR23 (Cutover Readiness)'s first slice, PR23A (Architecture & Operational Design), has since merged too (GitHub PR #122); this PR is the PR23 Owner Decision Closure round, in which the Repository Owner approved all six PR23 Owner Decisions (OD-PR23-1 through OD-PR23-6) per Recommendation. PR23B+ remains not started. See `docs/ROADMAP.md`, `docs/ROADMAP_STATUS.md`, and `docs/DECISION_LOG.md` for current status.** **Update (2026-08-25, this same historical paragraph): the PR23 Owner Decision Closure round has since merged too (GitHub PR #123, squash SHA `22ec7a25d686b0cd37d2a366172cb31a49eebff8`, current baseline), releasing the fail-closed PR23B+ implementation-authorization gate. PR23B (Cutover Readiness Evidence Foundation) implementation is now in progress, not yet merged — an additive backend-only persistence foundation (`CutoverReadinessRun`, migration `0021_cutover_readiness`, minimal Administrator-only API) implementing OD-PR23-6's approved persisted-evidence model, with no readiness-gate evaluation, Go/No-Go logic, or frontend. See `docs/ROADMAP.md`, `docs/ROADMAP_STATUS.md`, and `docs/DECISION_LOG.md` for current status.**

## Roadmap PR21 SDC scope decision: SDC EXCLUDED for V1; combined canonical adapter authorized

Roadmap PR21 (Legacy Receive and Issue History Import)'s last remaining Owner Decision — whether `ข้อมูลการส่ง SDC`/`ข้อมูลการรับ SDC` are authoritative V1 transaction-history input — is **RESOLVED: EXCLUDED**. The Owner selects the four already-confirmed canonical sheets (`Orders ยืมเครื่อง` + `ข้อมูลส่งเครื่องมือ` for Issue, `Orders คืนเครื่อง` + `ข้อมูลรับเครื่องมือ` for Receive) as the sole PR21 V1 authoritative source contract. **This is a source-authority decision, not a claim that SDC has been proven row-level-equivalent to the canonical sheets** — measured evidence (non-blank/distinct counts match; large trailing blank-row blocks explain the raw total-row divergence) narrows the ambiguity but a row-by-row diff was never performed, and this decision does not pretend otherwise. All seven PR21 Owner Decisions (OD-PR21-0 through OD-PR21-6) are now RESOLVED — none remain open.

By the time this decision was recorded, PR21-Foundation (GitHub PR #100), PR21A — Historical Event Schema / Provenance Foundation (GitHub PR #103), PR21B — canonical Issue parser (GitHub PR #104), and PR21C — canonical Receive parser (GitHub PR #105) were all already merged, each an unregistered internal component (never calling `register_adapter()`) so that a real `ImportSession` could not reach `validated`/dry-run/execute from either side alone — the deliberate safety mechanism that made merging Issue and Receive independently, before SDC was resolved, safe. With SDC now excluded, this decision **authorizes** (without itself implementing) composing those two parsers into the production `legacy_transaction_history` `ImportAdapter`, under a mandatory atomicity rule: one workbook → one `ImportSession` → one `ImportSource` → all four canonical sheets → one aggregate all-or-nothing validation decision (an Issue-only or Receive-only `DryRunPlan` is explicitly forbidden). It does not authorize Issue↔Receive pairing (still PR22-or-later's responsibility) or `LegacyEquipmentEvent` execution.

A separate, already-known blocker is recorded alongside this decision: the approved workbook (~19.7 MiB) exceeds the generic 10 MiB import upload limit. A PR21-specific bounded upload allowance (`PR21_MAX_UPLOAD_BYTES = 32 MiB`, or an equivalent dataset-specific mechanism) is authorized, mirroring the same architectural principle already applied to `PR21_MAX_WORKSHEET_COUNT = 32` (PR21B fix round) rather than raising the generic limit for every dataset.

The next implementation slice was split for reviewability: **PR21D1 — Combined Canonical Adapter + Source Admission** (authorized at the time this decision was recorded) composes the parsers, adds the upload allowance, registers the adapter, and wires the all-or-nothing validation gate. **PR21D2 — Historical Event Execution** (the `LegacyEquipmentEvent` INSERT step) was blocked on PR21D1 actually merging at the time this decision was recorded. **Both have since merged — see the newer entry above ("Roadmap PR21 complete: PR21D1–PR21E merged").**

Documentation-only — no backend, frontend, migration, test, or CI file changed. See `docs/DECISION_LOG.md` ("Roadmap PR21 Owner Decision Closure Round 3") for the full decision record and `docs/design/PR21_LEGACY_TRANSACTION_HISTORY_IMPORT_PLAN.md` §6.5/§45/§46/§47/§55 for the governing design-document text. **PR21A, PR21B, and PR21C were complete at the time this entry was written; PR21D1 was authorized but not yet started. This was accurate at the time — PR21D1, PR21D2, PR21E0, and PR21E have since all merged; see the newer entry above.**
