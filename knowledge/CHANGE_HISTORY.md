...
The next implementation slice is split for reviewability: **PR21D1 — Combined Canonical Adapter + Source Admission** (authorized, not yet implemented) composes the parsers, adds the upload allowance, registers the adapter, and wires the all-or-nothing validation gate. **PR21D2 — Historical Event Execution** (the `LegacyEquipmentEvent` INSERT step) remains blocked on PR21D1 actually merging. PR21E (frontend) and PR21F (Governance Sync) remain downstream, unaffected beyond this split.

Documentation-only — no backend, frontend, migration, test, or CI file changed. See `docs/DECISION_LOG.md` ("Roadmap PR21 Owner Decision Closure Round 3") for the full decision record and `docs/design/PR21_LEGACY_TRANSACTION_HISTORY_IMPORT_PLAN.md` §6.5/§45/§46/§47/§55 for the governing design-document text. **PR21A, PR21B, and PR21C are complete. PR21D1 is authorized but not started. PR21D2, PR21E, and PR21F remain blocked.**
